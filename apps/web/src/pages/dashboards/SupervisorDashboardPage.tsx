...
export const SupervisorDashboardPage: React.FC<SupervisorDashboardPageProps> = ({ onNavigate, projectId }) => {
  const { user, profile, refreshProfile } = useAuth();
  const isPending = profile?.status === 'PendingVerification';

  // Verification Form State
  const [documentUrl, setDocumentUrl] = useState('storage/faculty-credentials/faculty-id.pdf');
  const [institutionDomain, setInstitutionDomain] = useState(
    profile?.institution ? `${profile.institution.toLowerCase().replace(/\s+/g, '')}.edu` : 'stanford.edu'
  );
  ...
  const handleSubmitVerification = async (e: React.FormEvent) => {
    ...
    if (!documentUrl || !institutionDomain) {
      setDocError('Please provide both document URL and institutional domain.');
      return;
    }
    setIsSubmittingDoc(true);
    setDocError(null);
    try {
      await api.submitSupervisorVerification({ documentUrl, institutionDomain });
      setSubmitSuccess(true);
      await refreshProfile();
    } catch (err: any) {
      setDocError(err.message || 'Failed to submit verification.');
    } finally {
  ...
  // Compute stats
  const totalStudents = projects.reduce((sum, p) => sum + (p.membersCount || 0), 0);
  ...
  const renderDashboardHome = () => (
    <div className="space-y-8">
      ...
      {isPending && (
        <div className="rounded-3xl bg-gradient-to-r from-amber-950/40 via-[#141824] to-[#0E1118] border border-amber-500/30 p-6 relative overflow-hidden shadow-xl">
          <div className="flex flex-col lg:flex-row gap-6 items-start justify-between">
            <div className="space-y-2 max-w-2xl">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-semibold">
                ...
                <span>Pending Institutional Review</span>
              </div>
              <h2 className="text-xl font-bold text-white">Faculty Credentials Required</h2>
              <p className="text-xs text-slate-300 leading-relaxed">
                As a Principal Investigator or Thesis Supervisor, institutional verification ensures authorized faculty can govern research teams and approve milestone submissions.
              </p>
            </div>

            <div className="w-full lg:w-96 p-4 rounded-2xl bg-[#0E1118]/90 border border-slate-800 shrink-0">
              {submitSuccess ? (
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs space-y-1 text-center">
                  <CheckCircle2 className="w-5 h-5 mx-auto mb-1" />
                  <div className="font-semibold">Verification Request Submitted</div>
                  <p className="text-slate-400 text-[11px]">Administrator is reviewing your institutional credentials.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmitVerification} className="space-y-3">
                  <div className="text-xs font-semibold text-slate-300">Submit Verification Document</div>
                  {docError && (
                    <div className="p-2 rounded-lg bg-rose-500/10 text-rose-400 text-[11px] flex items-center gap-1.5">
                      <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                      <span>{docError}</span>
                    </div>
                  )}
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1">Faculty ID Document URL</label>
                    <input
                      type="text"
                      value={documentUrl}
                      onChange={(e) => setDocumentUrl(e.target.value)}
                      placeholder="storage/credentials/faculty-id.pdf"
                      className="w-full bg-[#141824] border border-slate-700/60 rounded-xl px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-slate-400 mb-1">Institutional Domain</label>
                    <input
                      type="text"
                  ...
                  <button
                    ...
                    className="w-full py-2 px-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-md flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                  >
                    {isSubmittingDoc ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <UploadCloud className="w-3.5 h-3.5" />}
      ...
      {/* Welcome Banner */}
      <div className="rounded-3xl bg-gradient-to-r from-violet-950/50 via-[#0E1118] to-indigo-950/30 border border-violet-500/20 p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <UserAvatar
              ...
              role={profile?.role}
              size="xl"
              className="ring-2 ring-violet-500/30 shadow-lg"
            />
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-400">
                {profile?.institution} · {profile?.department} · <span className="text-violet-400 font-medium">Principal Investigator</span>
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2.5 shrink-0">
            ...
            >
              ...
              <span>New Supervised Project</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Supervised Projects', value: projects.length, icon: Layers, color: 'text-violet-400', bg: 'bg-violet-500/10 border-violet-500/20' },
          { label: 'Research Students', value: totalStudents, icon: GraduationCap, color: 'text-cyan-400', bg: 'bg-cyan-500/10 border-cyan-500/20' },
          { label: 'Total Tasks', value: totalTasks, icon: ClipboardCheck, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
          { label: 'Completed', value: completedProjects, icon: CheckCircle2, color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/20' },
        ].map((stat) => (
      ...
      {isLoading ? (
        ...
        <div className="py-24 max-w-lg mx-auto text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">No supervised projects yet</h2>
          <p className="text-xs text-slate-400 leading-relaxed">
            Click "New Supervised Project" above to create your research group workspace.
          </p>
        </div>
      ) : (
        <div>
          <h2 className="text-sm font-bold text-white mb-4 flex items-center space-x-2">
            ...
            <span>Supervised Research Projects</span>
          </h2>
                ...
                <div className="flex items-center space-x-3 text-[11px] text-slate-500">
                  {project.membersCount !== undefined && (
                    <span className="flex items-center space-x-1">
                      ...
                      <span>{project.membersCount} students</span>
                    </span>
                  )}
      ...
      {/* Literature & Required Reading Oversight Card */}
      <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-indigo-950/20 via-surface-2 to-violet-950/20 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <span>Literature Oversight & Required Reading</span>
            ...
            <p className="text-xs text-slate-400 leading-relaxed max-w-xl">
              Assign required literature to project researchers, link publications directly to workspace tasks, review student structured analysis fields, and export BibTeX bibliographies.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-lg shadow-indigo-600/30 transition-all hover:scale-105 shrink-0 flex items-center gap-2"
        >
  ...
  return (
    <WorkspaceLayout
      ...
      headerProps={{
        ...
        onOpenNewProjectModal: () => setIsCreatingProject(true),
